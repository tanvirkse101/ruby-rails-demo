...
class UsersIndexAdminTest < UsersIndexAdmin
  ...
  test "should display only activated users" do
    # Disable the first user on the page.
    User.paginate(page: 1).first.toggle!(:activated)

    # Re-fetch the /users page and confirm that deactivated user is not displayed.
    get users_path
    assert_template 'users/index'

    # Confirm that all displayed users are activated.
